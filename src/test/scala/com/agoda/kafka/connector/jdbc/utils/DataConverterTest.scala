...
import java.sql.{ResultSet, ResultSetMetaData, Types, Date => SqlDate, Time => SqlTime, Timestamp => SqlTimestamp}
...
class DataConverterTest extends WordSpec with Matchers with MockitoSugar {
  ...
  "Data Converter" should {
    ...
    "convert record from result set" in {
      ...
      r.map(_.getBytes("BINARY").toSeq) shouldBe Success(Seq(50, 50))
      r.map(_.getBytes("VARBINARY").toSeq) shouldBe Success(Seq(50, 51))
      r.map(_.getBytes("LONGVARBINARY").toSeq) shouldBe Success(Seq(50, 52))
